...
def toursWithLength(n): # Generates all valid tours of length n.
    ...
    return list(filter(lambda x : x[-1] == base, newTourList)) # We accept the turs which end at the base.
...
solut = mainFunction(n)
for tour in solut:
    print("Tour:   ", end="")
    for city in tour[0][:-1]:
        print(city + " -> ", end="")
    print(tour[0][-1]+'.')
    print("Total balance: " + str(tour[1]) + '.')
